...
export async function calculateReleasePlan(options) {
	...
	const config = resolveReleaseConfig({ packageManager: options.packageManager });
	const baseSha = git(cwd, ["rev-parse", "HEAD"]);
	const expectedVersion = options.expectedVersion
		? assertReleaseVersion(options.expectedVersion)
		: undefined;
	const lastTag = latestStableTag(cwd, expectedVersion);
	const currentVersion = validateCurrentVersionFiles(cwd, config).version;
	if (expectedVersion) {
	...
	const notes = generatedNotes.trim() ? generatedNotes : `## ${version}\n\nMaintenance release.\n`;
	return {
		baseSha,
		nextVersion: version,
		notes,
